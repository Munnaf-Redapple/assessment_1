...
let profileDetailsFunction = async (req, res) => {
    ...
    try {
        let record = await UserModel.find({ status: 'active', _id: mongoose.Types.ObjectId(req.user.user_id) });
...
let updateFunction = async (req, res) => {
    ...
    try {
       ...
       let userDeatils= await UserModel.findOneAndUpdate({ _id: mongoose.Types.ObjectId(req.user.user_id) }, updateUserData, { new: true });
